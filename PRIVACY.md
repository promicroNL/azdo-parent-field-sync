...
During a pipeline run, the task reads configured Azure Boards work items and updates mapped fields in the same Azure DevOps organization and project. Work item field values are processed in the pipeline agent's memory and sent only to that Azure DevOps organization. The extension does not operate an external service, store work item data, or transmit it to Promicro or another third party.
